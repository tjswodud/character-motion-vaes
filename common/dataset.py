...
for file in dataset_list:
    ...
    facing_vector = np.cross(average_vectors, np.array([[0, 1, 0]]))
    ...
    velocities = np.concatenate((np.expand_dims(np.zeros((velocities.shape[1], velocities.shape[2])), axis=0), velocities)) # (frame, 22, 3)
...
np.savez('../environments/mocap_pfnn_2', data=data, end_indices=end_indices)
